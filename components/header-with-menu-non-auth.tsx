...
import { Categorias } from "@/app/layout";


function HeaderWithMenuNonAuth({ categories }: { categories?: Categorias[] }) {
    ...
    const menu = [
        ...
        { name: "Categorias", link: "?category=all", children: categories },
        { name: "Contato", link: "/contactos", },
    ];
    const socialIcons = [
                                    ...
                                    <div className="absolute left-0 mt-2 hidden group-hover:flex flex-col bg-white shadow-md rounded-lg border border-gray-200 w-48 opacity-0 group-hover:opacity-100 transform scale-y-0 group-hover:scale-y-100 origin-top transition-all duration-300">
                                        <ul className="py-2">
                                            {item.children.map((child) => (
                                                <li key={child.name}>
                                                    <Link href={`?category=${child.name}`} className="block px-4 py-2 text-gray-700 hover:text-green-500 hover:bg-gray-100 transition">
                                                        {child.name}
                                                    </Link>
                                                </li>
                                            ))}
                                ...
                                <div key={item.name}>
                                    ...
                                    {item.children && submenuOpen[item.name] && (
                                        <div className="pl-4 mt-2 flex flex-col space-y-2">
                                            {item.children.map((child) => (
                                                <Link key={child.name} href={`/categorias?type=${child.name}`} className="text-gray-600 hover:text-green-500 transition">
                                                    {child.name}
                                                </Link>
                                            ))}
                                        </div>
