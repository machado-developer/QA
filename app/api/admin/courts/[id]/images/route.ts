...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        const courtExist = await prisma.court.findUnique({
            where: { id: courtId }
        })

        if(!courtExist){
            return NextResponse.json({ message: "Quadra nao encontrado" }, { status: 404 });

        }
        const data = await prisma.courtImage.findMany({
            where: { courtId }
        });
